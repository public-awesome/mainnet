...
_Planned Start Time: October 29th at 15:00 UTC._

_Gentx submission deadline: October 28th at 15:00 UTC._
...
This will build and install `starsd` binary into `$GOBIN`.
...
Only nodes that particpated in the testnet will be able to validate. Others will be able to join the validator set at a later date after the chain launches.
...
3. Download the pre-genesis file:
   ...
   ```bash
   curl -s  https://raw.githubusercontent.com/public-awesome/mainnet/main/stargaze-1/pre-genesis.json >~/.stars/config/genesis.json
   ```
...
4. Create the gentx, replace `<KEY-NAME>`:
   ...
   ```bash
   starsd gentx <KEY-NAME> 10000000000ustars --chain-id=stargaze-1
   ```
   ...
   ```bash
   Genesis transaction written to "/home/user/.stars/config/gentx/gentx-******.json"
   ```
...
- Copy the generated gentx json file to `<REPO-PATH>/stargaze-1/gentx/`
  ...
  ```bash
  cd mainnet
  cp ~/.stars/config/gentx/gentx*.json ./stargaze-1/gentx/
  ```

- Commit and push to your repo
- Create a PR onto https://github.com/public-awesome/mainnet
- Only PRs from individuals / groups with a history successfully running validator nodes and that have initial stargaze balance from the testnet will be accepted. This is to ensure the network successfully starts on time.

#!/bin/bash
set -eux
docker pull publicawesome/stargaze:beta
...
starsd testnet --v 4 --output-dir tmp/local-testnet --chain-id stargaze-1 --initial-staking-amount 500000000000 --stake-denom ustars --coins 600000000000ustars --docker-tag beta

for i in tmp/local-testnet/gentxs/*.json; do
    echo $i
...
do
    echo "$i"
    cp $STARGAZE_HOME/config/genesis.json "tmp/local-testnet/node$i/starsd/config/"
    i=$(($i+1))
done
